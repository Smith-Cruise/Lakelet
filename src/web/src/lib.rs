...
//! of the two it is as the `web_ui_bundled` cfg.
...
pub const MISSING_BUNDLE: &str =
    "web UI is not bundled in this build; run `pnpm -C web build` and recompile";

/// The same fact as [`MISSING_BUNDLE`], phrased for the one-line server
/// startup banner. Kept beside it so the two cannot drift apart.
pub const STATUS: &str = "not bundled (web/dist was missing at compile time)";

/// Whether a bundle was embedded.
///
/// `cfg!` rather than `#[cfg]` so both arms of every caller stay compiled and
/// linted no matter which kind of build this is.
