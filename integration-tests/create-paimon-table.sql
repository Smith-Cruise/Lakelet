CREATE DATABASE IF NOT EXISTS paimon.`paimon_db`;

DROP TABLE IF EXISTS paimon.`paimon_db`.`orders`;

CREATE TABLE paimon.`paimon_db`.`orders` (
    id INT,
    name STRING,
    amount DECIMAL(10, 2),
    dt STRING
...
INSERT INTO paimon.`paimon_db`.`orders`
VALUES
